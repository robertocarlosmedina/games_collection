...
def movimentoIA(board: list, jogador: int, verificaGanhador, start_pos = None):
    possibilidades = getPosicoes(board)
    melhor_valor = None
    melhor_movimento = None
    for possibilidade in possibilidades:
        board[possibilidade[0]][possibilidade[1]] = token[jogador]
        valor = minimax(board, jogador, verificaGanhador)
...
def minimax(board: list, jogador: int, verificaGanhador) -> int:
